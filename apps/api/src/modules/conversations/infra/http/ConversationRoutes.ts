...
export function registerConversationRoutes(router: Router, controller: ConversationController): void {
  ...
  router.get('/api/conversations/:whatsapp/context', authenticate, authorize(['clients:read']), (req, res) => controller.getContext(req, res))
  ...
  router.post('/api/conversations/read-all',          authenticate, authorize(['clients:read']),  (req, res) => controller.readAll(req, res))
